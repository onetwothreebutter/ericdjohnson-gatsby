...
export default function HomePage() {
  ...
  function restoreOrder(event) {
    ...
    new TimelineMax().to(
      $appendage,
      0.4,
      { bottom: '100vh', ease: Power2.easeIn },
      0
    )
  }
  ...
  function triggerAppendage() {
    ...
    new TimelineMax()
      ...
      .fromTo(
        ...
        { y: 0 },
        {
          y: homepageContainerDistance,
          ease: Power2.easeIn,
          onComplete: playAudio,
          onCompleteParams: ['foot-sound-effect'],
        },
        'moveText-=0.3'
      )
      .to(
        $easterEggVideoContainer,
        0.15,
        { y: 50, scaleY: 0.1, opacity: 0 },
        'moveText-=0.2'
      )
      .fromTo(
        ['footer', $homepageContainer, '.photo-focus'],
        0.1,
        { scaleY: 1 },
        { scaleY: 0.1 },
        'moveText-=0.1'
  ...
  function setupHomepageDrag() {
    ...
    var dragHomepageHandle = Draggable.create($dragHandle, {
      type: 'y',
      edgeResistance: 0.65,
      cursor: 'ns-resize',
      bounds: $homepageHeading,
      throwProps: true,
    })[0]
    ...
    function onDrag() {
      ...
      if (scaleY >= 0.9) {
        if (dragHomepage === null) {
          ...
          TweenMax.to(['footer', '.photo-focus', $homepageContainer], 1, {
            scaleY: 1,
            ease: Elastic.easeOut.config(2, 0.3),
          })
          playAudio('cartoon-pop-sound')
          dragHomepage = Draggable.create($homepageContainer, {
            type: 'x,y',
            edgeResistance: 0.65,
            bounds: 'body',
            throwProps: true,
  ...
  function addAudioFile(fileName, className) {
    var $audio = $(
      '<audio class="' + className + '" src="' + fileName + '"></audio>'
    )
    $('body').append($audio)
  }
